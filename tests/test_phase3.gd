...
# Stack resolution is LIFO: opp's Giant Growth, cast in response to Lightning
# Bolt, resolves first and pumps the bear to 5/5 before Bolt's 3 damage lands.
...
func _ready() -> void:
	...
	# opp's hardcoded AI casts Giant Growth in response, then auto-passes —
	# one pass_priority call from us surfaces both of opp's actions.
	...
	# 3 damage < 5 toughness: SBA doesn't kill the bear
