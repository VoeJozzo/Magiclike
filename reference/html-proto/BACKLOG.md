...
- **Indestructible should preserve marked damage, not clear it** — currently `engine.js:3591-3594` clears `c.damage = 0` when an indestructible would otherwise die from damage. MTG-correct behavior: damage stays marked until end of turn; indestructible only skips the death check. If indestructible is removed mid-turn (aura loss, ability loss, etc.), the marked damage should kill the creature at the next SBA. Today proto loses that history and the creature survives. Cross-ref: `docs/DIVERGENCE.md` item F2.
- **`gain_life` should refuse non-positive amounts** — currently the effect silently applies whatever amount is passed, including 0 (no-op) and negative (life decreases). MTG keeps "gain life," "lose life," and "take damage" as three distinct events with different triggers. Overloading `gain_life` with negative amounts conflates them. Align on Godot's warn-and-skip behavior; when proper "lose life" mechanics are needed, add a separate effect kind rather than overloading gain. Cross-ref: `docs/DIVERGENCE.md` item D4.
