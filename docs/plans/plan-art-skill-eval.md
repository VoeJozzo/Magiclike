...
**Tally (keepers):** treatment 3, control 5, no-keeper 3. Quality-noted frames: control 4, treatment 2. n=11 rounds; **8 keepers banked** (user wants 10 keepers before the control-vs-treatment discussion). Keeper sequence: serra(T), illusion(C), inferno(T), island(C), llanowar(C), mercurial(C), might(C), mind_control(T) — treatment broke control's 4-keeper streak. Composition finding (martyr_saint): the C4 treatment kept rendering the gain-life mechanic literally as a two-figure "she gives / he receives" transfer, which doesn't survive the 64×32 downscale; control's single-figure framing read cleaner. (Observation: control keeps winning blind best-vs-best; treatment's edge from serra/inferno isn't recurring.)
