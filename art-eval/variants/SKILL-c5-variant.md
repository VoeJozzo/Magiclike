...
### Evaluator-mode self-critique — judge the canvas, not your intent

Before nominating a best roll or deciding what to iterate, re-examine your own generations *as if someone else made them and you don't know what they were going for.* You made them holistically and can't un-know your *intent* — so you "see" the fireball-striking-the-victim you meant, not the "guy and a fireball" you rendered. Switch stances: assess only what is literally on the 64×32 canvas.

Run it explicitly. Score each candidate against: **mechanic-enactment** (does the canvas *alone*, with zero knowledge of intent, show the rule happening?), **silhouette legibility**, **color-identity match**, **scene-not-sprite**, and **gestalt** (does the whole read as the thing at size?). Then **force a ranking** — ordered list, single best named, one line on *why it beats #2.* "They're all good" is not a ranking; it's the avoidance this exists to break.

Expect the rubric to disagree with your gut: the keeper is often a roll you'd have discarded (generation outruns self-selection). When "the roll I enjoyed making" and "the roll that wins the rubric" diverge, trust the rubric. The flaw you *name* here ("the off-hand reads as a third arm," "the beam never connects to the victim") is exactly what the next seed-locked tweak or inpaint targets — critique first, iterate at the named flaw, never reroll blind.
