...
- If the attacker has [[900-keywords|menace]], the defending player must assign **at least 2 blockers** to it. A block assignment that leaves a menace attacker with exactly one blocker is **illegal**: the engine rejects it when blocks are confirmed, so the defender cannot finalize a lone block on a menace attacker. (**Godot only:** a damage-time fallback that treats a lone-blocked menace attacker as unblocked is retained as a safety net for imported or otherwise stale states. The html-proto has no such fallback — if a stale lone block arises anyway, e.g. the second blocker dies during the block-step priority window, the proto resolves it as a **normal block**, matching real MTG. See `docs/DIVERGENCE.md` C3; audit A2-11.)
