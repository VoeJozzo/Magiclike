...
Pitched, unbuilt: *Patient Haunt* ({1}{W} Spirit 1/1 flying, dies → +1/+1 sticker) · *Thrice-Buried Saint* ({2}{W} Spirit Cleric 2/2, dies → sticker; 3+ stickers → flying + lifelink — needs one small "count own stickers" static, the only residual gap in the sticker grammar). Build-time checks: mid-battle recursion semantics; whether player slots cap sticker count.
