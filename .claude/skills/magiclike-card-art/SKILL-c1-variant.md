...
**Elements present ≠ relationship shown.** The most common way an "enacting" direction still fails: all the right *elements* are in frame — caster, bolt, victim — but nothing shows them *acting on each other*. The eye reads "a wizard and a fireball," not "a wizard hurling a fireball at a man." At 64×32 the causal link has to be composed on purpose or it doesn't survive. Three levers supply it:
- **A connecting element along the action's vector** — the beam, arc, spray, or trail that physically joins source to target (Char's fire bends back onto the caster; Doom Blade's void runs *through* the soldier).
- **Reaction, not just presence** — the caster braces/lunges/recoils; the victim flinches, buckles, is flung or torn. A neutral figure *beside* an effect is decoration; a figure *reacting to* it is caught in it.
- **Directional consequence** — debris, blood, cinders, frost thrown *along* the force's vector, not scattered radially. Direction is what says "this came from there and hit here."

**The cover test:** mask either the source or the target. Does the remaining half still imply "X is doing this to Y"? If each half is independently inert — a guy who could be standing anywhere, a blast that could be hitting anything — you have two coexisting sprites, not an enacted mechanic. Fix it before generating.
