...
| 7 | symmetricize decomposition (§3.8) | ⏸ **NOT done** — symmetricize is still a monolithic `{kind:"symmetricize"}` effect (handler at `engine.js`); the §3.8 decomposition to `stat_boost` + `cost_mod` snapshot stickers via `apply_sticker` remains. (Earlier marked "already done" in error — corrected after audit.) |
