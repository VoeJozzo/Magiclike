...
- **Zone routing.** Cards entering a graveyard go to their **owner's** graveyard everywhere (the stolen-card rationale is comment-documented). Battlefield arrivals through `placeCardOnBattlefield` mint a fresh iid (the §3.7 rule) and set summoning sickness; the cast-arrival path keeps the stack object's iid, and `fetchLibraryToBattlefield` has its own arrival path (land-only today). Reanimation from graveyard/exile mints fresh and sets sick (test-pinned). Tokens cease to exist on every leave path.
