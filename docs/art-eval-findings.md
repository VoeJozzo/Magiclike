...
## C2 A/B #16 — veil_of_mists (blue bounce: return creature to hand) [harness-driven, preflight PASS]
- NEITHER arm captured the mechanic (bounce/vanishing); both hit subject-prior dominance ("armored warrior standing in misty landscape" beat "creature dissolving away"). Control self-reported only 1/10 legibly enacting bounce.
- **User gold: "none really captured it; 1.x came SLIGHTLY closer but still meh; 1.x has some really good art."** Decode: label 1 = arm_a = **CONTROL**. => marginal CONTROL lean (not decisive, no keeper). Saved 3 SLUSH (good-art-not-on-mechanic): `veil_of_mists_1.04/1.05/1.07_SLUSH_*` (control arm; seeds 1820118767, 972161303, 298837792).
- C2 tally (user gold): decisive = **9 C2** / **3 control**; marginal = gilded_seat (C2), veil_of_mists (control) -> 1 each side; inconclusive = dark_ritual, vile_edict. Decisive sign test still 9/12 (two-tailed p~=0.15). Takeaway forming: on cards where the mechanic is a "removal/vanish" abstraction with a strong standing-subject prior, BOTH arms struggle — a skill-content gap (subject-prior defeat) orthogonal to C2.

## C2 A/B #17 — vexing_ogre (4/2 red, ETB lose 1 life) [harness-driven, preflight PASS]
- NOTE: both agents hit the subscription session limit AFTER completing all 10/10 gens per arm; only the agent self-eval artifacts (BEST/log) were lost. Images + A/B integrity intact (preflight PASS, 0 byte-identical cross-arm pairs). Prompt text not captured for this round (documentation gap).
- Both arms = strong fierce red ogres in fire; C2(arm_b) worked in handler figures (vexing-downside hint), control(arm_a) pure menace.
- **User gold: "2.07 is the pick, but NO clear winner; 1.x might actually be slightly better overall; picked 2.07 on subjective factors."** Decode: 2.07 = arm_a = control; 1.x = arm_b = treatment. => WASH / INCONCLUSIVE (user's own pick was control but they lean the other branch slightly; not decisive either way). Saved pick `vexing_ogre_2.07_USERPICK-wash_*` (seed 1497524782).

## ===== C2 STATS ANALYSIS v2 (n=17 rounds, user-gold, judge-free) =====
TALLY: decisive 9 C2 / 3 control | marginal 1 C2 (gilded_seat) / 1 control (veil_of_mists) | inconclusive/wash 3 (dark_ritual, vile_edict, vexing_ogre).
  C2 decisive: horned_herald, knight_commander, bindspeaker, old_guardian, prey_upon, grave_charm, giant_spider, great_stag, clockwork_beetle.
  control decisive: lightning_bolt(no-fig), centaur_courser, nature_caller.
SIGN TEST (binomial, null p=0.5): strict decisive 9/12 -> one-tailed p=0.073, two-tailed 0.146. +marginals 10/14 -> one-tailed 0.09, two-tailed 0.18. NOT at p<0.05; direction strongly C2 (75% decisive).
KEEPER SIGNAL (secondary, arguably stronger): of A/B keepers pulled, nearly ALL are C2 (old_guardian, grave_charm, giant_spider, great_stag, clockwork_beetle); only control keeper = nature_caller. C2 has produced every recent shippable frame.
MAGNITUDE ASYMMETRY (sign test ignores this): C2 owns ALL emphatic verdicts ("the one", "not close", "easily"). Control's 3 wins are narrow/defensive: no-figure Bolt (stripped variant), centaur (BOTH arms failed rider-prior; control less-bad), nature_caller (its one clean rich-card win — a real counterexample to "C2 only-helps-rich").
CONTROL/NULL CLUSTER: degenerate/narrow cards (single icon, vanilla body) + mechanics that resist depiction & trip subject-prior dominance (veil_of_mists bounce wash, vexing_ogre wash) — a skill-content gap (subject-prior defeat) orthogonal to C2, hitting BOTH arms.
VERDICT: directionally strong + practically compelling (9-3, every recent keeper C2, owns all emphatic wins), but NOT formal-significance by sign test (p~0.07-0.15). C2 is a low-risk one-paragraph add that has NEVER decisively hurt a rich card -> reasonable merge candidate on cost/benefit, but borderline vs the user's stated "clear data" bar. To reach two-tailed p<0.05 from here: ~11/14, 12/15, or 13/17 decisive (a few more mostly-C2 cards). Paused at user request (API credits).

## ===== C2 OUTCOME — ADOPTED (PR #73, merged to dev) =====
After the n=17 pause above, C2 ("reach, don't hedge — the diagnosable-flaw principle") was
**merged into the live `SKILL.md`** (PR #73 "Card-art skill: adopt the reach, don't hedge
principle + 19 new card arts"). Decision basis: directionally strong (9-3 decisive, every
recent keeper a C2 roll, owns all emphatic verdicts), low-risk one-paragraph add that never
decisively hurt a rich card — merged on cost/benefit despite not clearing formal sign-test
significance. The eval scaffolding (`art-eval/`, this findings doc, the plan doc) was stripped
from that PR so dev got "skill change + usable art only"; it was **recovered from git history
onto `claude/pixellab-art-generation-ozmbb7`** (2026-06-15) to resume the flywheel on C5.
C2 cycle CLOSED (adopted). Remaining queued candidates: C3 (color-on-subject, n=1), C4
(ground-in-reality, untested), C5 (self-critique, staged below), C6 (base-form-first for
X-folk, untested).

## ===== C5 — "self-critique with the judge's stance" — ROUND #1 STAGED (2026-06-15) =====
First test of C5, the highest-leverage queued candidate (attacks the most-replicated finding:
**generation > self-selection** — the user's favorite was repeatedly a roll the agent itself
discarded — plus artist-misses-own-flaws).

**Treatment text** (added to control = current live SKILL.md, post-C2; full file at
`art-eval/variants/SKILL-c5-variant.md`, signature "Evaluator-mode self-critique"):
> Before nominating a best roll or deciding what to iterate, re-examine your own generations
> *as if someone else made them and you don't know what they were going for.* ... Switch
> stances: assess only what is literally on the 64×32 canvas. Run it explicitly — score each
> candidate on mechanic-enactment / silhouette / color-identity / scene-not-sprite / gestalt,
> then **force a ranking** (best named + why it beats #2). Expect the rubric to disagree with
> your gut (keeper is often a roll you'd discard). The flaw you *name* is what the next
> seed-locked tweak / inpaint targets — critique first, iterate at the named flaw, never reroll
> blind.

**C5-specific metric** (differs from C1/C2): C5 is NOT a prompt-content change, so full-pool
generation quality should be ~equal between arms. The load-bearing read is **nominee-vs-user-gold
alignment** — does the self-critiquing arm crown the roll the user crowns? Both arms record their
nominee internally; user judges the blind full-pool; then compare each arm's nominee to the pick.

**Round #1 setup (ready to fire):**
- Card: `hypnotic_specter` (2/2 flying; on attack, opp discards). Chosen deliberately (not blind
  random): the abstract discard mechanic tempts a cool-but-mechanic-empty nomination = the exact
  output-vs-intent failure C5 targets. Documented departure from the harness's blind pick, justified
  by isolation (degenerate cards produce low-variance washes with nothing to self-critique between).
- Run: `skillab-c5-hypnotic_specter`. Shared 10-seed pool (committed at run-root seeds.json):
  1733774096, 1948976646, 2118073012, 816736794, 595462494, 1399165970, 1694428303, 2105516246,
  12020585, 1235452775.
- Harness generalized: `ART_EVAL_CAND=c5`; selftest PASS; init done; variant check OK.
