...
- **Type-system cutover: delete `type`/`sub`, migrate the ~50 readers to `governingType()`/`hasType()`** — v2.0.67 added an explicit `types[]` to every card *additively*: `type`/`sub` were kept because ~50 engine sites still read them directly (`tpl.type === 'Creature'`, `card.sub.split(...)`, etc.). That leaves a **redundant-state window** — a hand-edit that updates `type` but not `types[]` (or vice-versa) would diverge silently. `tests/test_types_identity.js` pins the two forms equivalent *today*, which guards the window but doesn't close it. The cutover: (1) migrate the direct readers (`grep -nE "\.type\s*[=!]==|\.sub\b" js/*.js`, minus `types.js`) to `governingType(c)` / `hasType(c, X)`; (2) drop `type`/`sub` from `card.json` (keep `types[]` as the sole source of truth); (3) retire the equivalence half of `test_types_identity` (the accessors become the definition, not a mirror of legacy fields). Do it as its own focused PR — it's a second large sweep and shouldn't ride along with unrelated work. Until then, `types[]` is authoritative (typesOf prefers it) and the legacy fields are a compatibility shim, not a parallel truth.
