...
keyed by sheet filename only; no source identity anywhere.

This is the PORTABLE form. The standalone index.html needs the sheet PNGs on
disk (under SHEETDIR); the bundle carries them inside it, so it survives even
after the raw sheets are cleaned from the working tree.

Usage:  python3 bundle.py [cand]
"""
...
SHEETS = repo_root() / "docs" / "art-eval-sheets"
