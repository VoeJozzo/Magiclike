...
- **Q1 — folder/card-id collision. DECIDED: keep Godot's snake_case names; rename the colliding proto folders to match.** The snake_case wire decision (PROTOCOL.md) governed *field* names, not card *ids*, so proto's camelCase/abbreviated ids didn't violate it — but Godot's names are correct and unabbreviated, and adopting them also fixes two mislabeled proto folders (`hastyOgre` is Raging Goblin, `cloudGiant` is Cloud Pegasus). The 5 basic lands already match verbatim. The 16 folders to rename (proto → Godot id): `bolt`→`lightning_bolt`, `bears`→`grizzly_bears`, `counter`→`counterspell`, `growth`→`giant_growth`, `serra`→`serra_angel`, `salve`→`healing_salve`, `airel`→`air_elemental`, `spider`→`giant_spider`, `bloodKnight`→`blood_knight`, `whiteKnight`→`white_knight`, `goblinRaider`→`goblin_raider`, `emberDrake`→`ember_drake`, `mistDjinn`→`mist_djinn`, `angelOfDawn`→`dawn_angel`, `hastyOgre`→`raging_goblin`, `cloudGiant`→`cloud_pegasus`. The 10 Godot-only cards (`bear_cub`, `bloodlust_berserker`, `goblin_duelist`, `gray_ogre`, `hill_giant`, `pyromaniac`, `trained_armodon`, `vampire_nighthawk`, `walking_wall`, `wind_drake`) get authored as new snake_case proto folders. Update each renamed folder + its `_manifest.json` entry + any proto deck/test references to the old tplId, in one commit. **Open sub-question (scope):** whether to also normalize the remaining ~237 camelCase proto ids to snake_case now — recommended as a *separate* backlog sweep, not part of Slice 1 (the mixed-convention pool is harmless since ids are opaque).
...
- **Q3 — `extraManaColors`. No action here; resolved by effects-plan §3.9.** The wire still carries `mana` + `extraManaColors`; §3.9's mana deep-clean (lands → `add_mana` ability model) reworks it. Keep `JsonCardLoader`'s current handling for Part 1; let §3.9 change it.
