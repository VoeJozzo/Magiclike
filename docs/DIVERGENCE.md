...
| B6 | Auto-pass when no legal action | Not implemented — `_settle_state` only auto-passes for AI; human player must explicitly pass priority even when they have nothing to do | Implemented via `hasNoAction(who)` at engine.js:5177 plus `skipApEndStep` at engine.js:5196. Any priority holder (including human) with no castSpell / activateAbility / playLand actions has priority auto-passed. Active player's empty-stack END priority is also auto-skipped. | 🟡 | **godot:** add equivalent click-saving auto-pass. Add a `_has_no_legal_main_action(player_key)` helper that filters `get_legal_actions` to cast/activate/play-land kinds. In `_settle_state`, when actor is `"you"` and the helper returns true (and no awaiting-state is active), call `_dispatch_action(Action.make_pass_priority())` instead of breaking. Also add the empty-stack END-step skip. Same logic, same place — keeps the auto-pass landscape self-contained. |
