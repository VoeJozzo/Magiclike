...
// no cards.
// Runs before the first paint — otherwise cards flash with default fonts
// for a frame before the saved override applies.
SETTINGS.applyFontsToRoot();

loadCards().then(() => {
  // Boot validation: surface typos in composable trigger conditions / event
  // kinds and effect kinds / target filters at startup, not at runtime when
  // a trigger or effect fails.
  validateAllCardConditions(CARDS);
  ENGINE.validateAllCardEffects(CARDS);
  // The generated-trigger data tables (Codex generator + Mercurial pool)
  // sit outside both card validators — sweep them too.
  ENGINE.validateGeneratedTriggerTables();
  // A TPLID_RENAMES key reused as a LIVE card id makes picklog silently
  // rewrite that card's pick history on every load — surface it at boot.
  ...
  // Every EFFECTS handler must be classified for AI valuation and have
  // card-text. A miss here means a future kind would silently score 0 / show
  ...
  // Which tribal themes the synergy graph can serve and which are thin —
  // the thin list doubles as a card-design TODO list.
