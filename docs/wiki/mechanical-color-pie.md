...
- **White excludes rather than crusades** *(settled 2026-07-13, held lightly)*: white's answers are fees, arrests, and universal rules (Letter of Passage's bail, Gate Warden's toll), and its taxes never read the caster's colors. No formal test — "we'll know it when we see it" (Joe); mythic enmities (angels vs. demons) are fair game, and "deal 3 damage to all Goblins" is not a race-relations statement.
- **Spell-riders: frames-are-free applies; tribal home OPEN**: the payload obeys the pie (Wave 2 already complies). Whether riders become a tribe/class signature (Shaman-conductor's static form? Cleric liturgy? nobody's?) is undecided — Wave 2's type assignments carried no flavor intent, so its customers are not evidence.
