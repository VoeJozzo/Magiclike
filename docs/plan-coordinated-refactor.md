...
| D | [`plan-effects-refactor.md`](plan-effects-refactor.md) | 38→19 atomic effects + `target()`/`chooses()` (§3.5) + sticker (§3.8) + mana deep-clean (§3.9) + staple cleanup (§3.10) + rip (§13); **carries card-data Part 2** (full field/effect/trigger migration of all cards) | Both | L (~64–69h) |

Rough total **~115–130h (~3–4 weeks)** — sliceable; each slice below leaves both engines runnable and test-green.
...
**Slice 3 — Effects (D).** The big one: register the 19 atomics, the `target()`/`chooses()` targeting model (§3.5), the sticker pipeline (§3.8), the mana deep-clean (§3.9 — `extraManaColors` retired here, which resolves card-data **Q3**), the staple cleanup (§3.10), rip (§13), and **card-data Part 2** — the full field + effect/trigger migration of all cards, which also deletes `JsonCardLoader`'s remap tables (dispatch keys go fully snake_case). `flicker` decomposes immediately; `exile_until_eot` waits on **B4** (delayed-trigger machinery), added as a sub-step.
...
- **Q1 (Slice 1)** — folder/id collision (`bolt` vs `lightning_bolt`). Pin before Slice 1 touches card addressing. (Sequencing here doesn't depend on the answer.)
- **Q2 (Slice 1/3)** — modal-card JSON shape. Blocks only modal cards' playability.
