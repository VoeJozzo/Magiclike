...
| # | Area | Godot | Proto | Tag | TO-DO |
|---|---|---|---|---|---|
| C1 | **Multi-blocker damage assignment** | Dumps all damage on `blockers[0]` (engine.gd:1080) | Smart distribution — sorts blockers by kill-value, indestructibles last, assigns minimum lethal to each in order (`dealCombatDamage`, engine.js:5257-5383) | 🔴 BIG | **godot:** harmonize to proto's smart-distribution algorithm (rules §803). |
...
| C3 | Menace single-blocker handling | Illegal: rejected at block confirmation (`_legal_confirm_blocks` → `_menace_blocks_are_legal`; `_prune_single_menace_blocks`). Damage-time collapse retained as a guard for imported/stale states. | Illegal at declaration (engine.js:6050-6059) | ✅ Aligned | **already-aligned** — both reject a lone blocker on a menace attacker at declaration/confirmation. |
...
| C7 | Removal from combat (leave-play / control change) | Not implemented — combat lists are only reset wholesale at end of combat; consumers findCard-guard stale entries | `removeFromCombat(iid)` (engine.js, audit A2-3/A2-5, v2.1.22–23): the leave-battlefield funnel AND `change_control` prune `G.attackers` and retire `G.blockers` entries; a blocked attacker stays blocked when its blocker leaves (510.1c, via tombstoned blocker keys) | 🔴 (when porting bounce/flicker/steal-mid-combat interactions) | **godot:** implement removal-from-combat (rules §801 "Removal from combat") when the port grows mid-combat zone-change or control-change interactions. |
...
| # | Area | Godot | Proto | Tag | TO-DO |
|---|---|---|---|---|---|
| D0 | Stack resolution order + post-cast priority | LIFO via `Stack.push` / `Stack.pop_top` (engine.gd:628, 669). Caster retains priority after cast (engine.gd:685-686 in `_do_cast_spell`), per MTG 117.1c. | LIFO via `G.stack.push` / `G.stack.pop` (engine.js:5007, 5030). **Post-cast priority passes to the NON-caster** (`pushOnStack`, `pushTriggerEntry`), so the opponent gets the first response window; the caster regains priority only after the opponent passes. | ✅ LIFO same / 🟡 priority diverges (intentional) | **Adjudicated — intentional proto design (Joe's ruling, PR #98, 2026-06-10; audit A1-1):** "holding priority is an edge case that leads to a lot of clicking," so the proto deliberately drops MTG 117.1c and hands post-cast priority to the opponent. Canon now documents this (§603, §702.5). Godot currently retains priority per 117.1c — whether the port harmonizes to the proto design is the port's decision, not a proto bug. |
...
| D8 | Activated-ability resolution: mana fast-path vs `kind:'ability'` stack entries | Mana abilities skip the stack — `_do_activate_ability` (engine.gd:522-533) calls `Effects.resolve_one` directly when the source is a land, no stack push, controller retains priority. Currently only supports land mana abilities (non-mana activated abilities are Phase 6+). | Mana abilities: same fast-path (skip the stack, no priority required). Non-mana activations: since v2.1.42 (audit A3-2 stackable infrastructure) they genuinely push a `kind:'ability'` stack entry — costs/targets locked at activation, §603 response window, §1006.1 resolution re-validation. An optional `stackable` boolean (absent → true; PROTOCOL §5/§6) selects the dormant inline arm; no card sets it yet, pending Joe's classification pass (`docs/plans/plan-stackable.md`). *(This row previously asserted the proto already stacked non-mana activations — that was false until v2.1.42; audit A3-2 caught it.)* | ✅ Aligned on the mana fast-path; 🔴 ability stack entries Godot-pending | **godot:** when the port gains non-mana activated abilities (Phase 6+), implement `kind:'ability'` stack entries + the `stackable` field from PROTOCOL §5/§6, not inline resolution. |
...
| E6 | Trigger budget (per-stack-episode cap) | None | 100 (`TRIGGER_DEPTH_CAP`, engine.js; counted and checked in `resolveTrigger`, reset when the stack empties). A **width budget, not nesting depth**: every trigger resolution since the stack last emptied counts toward the cap. Deliberate (design ruling, audit A3-3, PR #98, 2026-06-10) — the width count also bounds mutual A→B→A trigger loops a true depth counter would never catch. | 🟡 | **godot:** add the cap. The "bet on drain correctness" stance from CLAUDE.md doesn't survive contact with real card design — the user has already accidentally produced infinite-loop card combinations on proto. The cap costs one counter + one comparison; the defensive value (preventing a hung session, surfacing the bug at the right place) is high. Mirror proto's per-stack-episode **100-trigger budget** (NOT nesting depth — see canon §1008). CLAUDE.md "Patterns to NOT replicate" entry to be removed in the same commit. |
...
| F4 | SBA sweep ordering | Single-pass collect-and-apply, iterative repeat. **Capped:** `var safety := 20` iteration cap in `_run_sbas` (engine.gd). | Single-pass collect-and-apply, iterative repeat. **Uncapped** `while(true)` in `checkDeaths` — termination guaranteed structurally (dies/leaves triggers queue into `pendingTriggers` rather than resolving inline, so the battlefield strictly shrinks each iteration). | ✅ ordering Same / 🟡 cap differs | **already-aligned** on ordering. Cap asymmetry recorded per audit A1-18 (inverse of E6, where proto has the trigger-depth cap and Godot lacks it). Optional belt-and-braces cap in the proto: flag at triage with the parked step() refactor if symmetry is wanted. |
