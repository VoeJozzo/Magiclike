...
**Tally (keepers):** treatment 2, control 5, no-keeper 3. Quality-noted frames: control 4, treatment 2 (might_of_faith 1.05 + 1.07 — first treatment quality flags since inferno). n=10 rounds; **7 keepers banked** (user wants 10 keepers before the control-vs-treatment discussion). Keeper sequence so far: serra(T), illusion(C), inferno(T), island(C), llanowar(C), mercurial(C), might(C) — control has the last 4 in a row. Composition finding (martyr_saint): the C4 treatment kept rendering the gain-life mechanic literally as a two-figure "she gives / he receives" transfer, which doesn't survive the 64×32 downscale; control's single-figure framing read cleaner. (Observation: control keeps winning blind best-vs-best; treatment's edge from serra/inferno isn't recurring.)
