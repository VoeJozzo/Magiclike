...
- **1002.2** Zone tokens for `card_zone_change` are `hand`, `library`, `graveyard`, `exile`, `stack`, `battlefield`, plus the synthetic `none` (a card minted directly into a zone — a token onto the battlefield, steal's fresh instance into the thief's library — came from no prior zone). **Every genuine card move between zones is announced** (audit A3-6 build-out): battlefield arrivals and leaves, draws and tutors (`library → hand`), discards (`hand → graveyard`), mills (`library → graveyard`), casts (`→ stack`), counters and spell resolutions (`stack → graveyard`), and graveyard/exile recursion. A trigger authored against any zone pair — `card_moves(library, hand)`, `card_moves(hand, graveyard)` — now fires.
  - **1002.2a** Zone-movement events **do not exist before the game starts**: opening-hand placement and the forced mulligan are game setup, not moves, and emit nothing. (Compare MTG, where abilities don't function outside the game — but this is Magiclike's own rule.) The turn draw, including the first player's first, IS a move and is announced.
  - **1002.2b** Non-moves that deliberately emit nothing: rip/annihilate and a steal-consumed stack spell (the card **ceases to exist** — rip is the engine's no-trigger removal verb by design), staple consumption (a merge, not a move), control changes (the card never leaves the battlefield zone), and library shuffles (reordering within one zone).
