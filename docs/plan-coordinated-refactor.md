...
| D | [`plan-effects-refactor.md`](plan-effects-refactor.md) | 38→~22 atomic effects + `target()`/`chooses()` (§3.5) + sticker (§3.8) + mana deep-clean w/ full-convergence Godot land-as-ability (§3.9) + staple cleanup (§3.10) + rip incl. built `annihilate` (§13) + **AI-valuation lockstep redesign + boot coverage assertion (§8.1)**; **carries card-data Part 2** (full field/effect/trigger migration of all cards) | Both | L (~92–97h) |

Rough total **~143–158h (~4–4.5 weeks)** — sliceable; each slice below leaves both engines runnable and test-green.
