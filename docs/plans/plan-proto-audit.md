...
5. Pre-authorize the audit worktree's permission allowlist — **enumerate every predictable permission upfront**; the dry run serves as the completeness check. This includes **wiring the promptless push/PR transport** per IDENTITIES.md's documented upgrade: per-worktree gh credential helper, username-pinned to the bot (`extensions.worktreeConfig` + `git config --worktree credential.https://github.com.*`), plus a `gh-bot` wrapper script for API writes. No bespoke token-in-URL commands — mechanism beats instruction.
...
|---|---|---|
| Permission prompt stalls the run | **Enumerate every needed permission upfront** into the audit worktree's allowlist (git, `gh`, `node`, in-worktree writes); push/PR transport wired as **mechanism, not bespoke commands** — per-worktree gh credential helper username-pinned to the bot + a `gh-bot` wrapper script for API writes, so every overnight git/gh command is a plain allowlistable form (credential-juggling one-offs can never pass an allowlist — learned live 2026-06-10 when the plan-PR push itself burned a manual permission); the supervised dry run is the *completeness check* — it exercises the real paths and surfaces anything prediction missed, while Joe is awake | A code path the dry run never walked — fixed next morning |
