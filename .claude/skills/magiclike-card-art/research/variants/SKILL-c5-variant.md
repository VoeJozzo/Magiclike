...
### Diagnose-then-iterate — read the canvas honestly, fix the flaw you can name

Between generations — before you decide what to reroll, seed-lock, or inpaint — re-examine your latest rolls *as if someone else made them and you don't know what they were going for.* You made them holistically and can't un-know your *intent*: you "see" the fireball-striking-the-victim you meant, not the "guy and a fireball" you actually rendered. Switch stances and assess only what is literally on the 64×32 canvas.

Read the canvas against concrete failure axes — **mechanic-enactment** (does the canvas *alone*, with zero knowledge of intent, show the rule happening?), **silhouette legibility**, **color-identity match**, **scene-not-sprite**, and **gestalt at size** — but use them to *find what's broken*, not to crown a winner. From that read, **name the single most damaging concrete flaw** in plain words: "the off-hand reads as a third arm," "the beam never connects to the victim," "at 64px the wings dissolve into the background."

That named flaw is the target, and naming it is the whole point: your next move is a *seed-locked tweak or inpaint aimed squarely at it* — never a blind reroll hoping a fresh random draw lands better. Critique first, then iterate at the named flaw; repeat until the canvas earns it. The generation budget is for repairing diagnosed defects, not spinning the wheel. (Choosing the final keeper is not yours to force — keep every roll and nominate honestly; the discipline here is only that each *iteration* must fix something you could name.)
