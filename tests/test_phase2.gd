...
func _ready() -> void:
	...
	# Sorcery-speed cast with no target — the empty array is correct, not a bug.
	...
	# Phase machine: MAIN1 → COMBAT_ATTACK → COMBAT_BLOCK → COMBAT_DAMAGE → MAIN2
	# → END → CLEANUP → wrap → opp UNTAP → opp's whole turn auto-cycles via
	# _settle_state → wrap again → your UNTAP. AI-driven opp stops at every
	# priority-pass round rather than auto-cycling its whole turn, so we wait
	# for active_player_key to be back to "you" rather than counting passes.
	...
	# Goblin Raider's power is 2 (opp.life 20 → 18). Pass advances
	# COMBAT_ATTACK → COMBAT_BLOCK → COMBAT_DAMAGE, which fires
