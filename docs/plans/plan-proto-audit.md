...
*Agreed 2026-06-09 (grill-me interview, Joe + Fable); revised 2026-06-10 after a second grilling — workshop branch model, Just-Say-Go arming, trivia mini-PRs, judgment-documented ship gate. This doc is the contract for the audit campaign — overnight runner sessions execute against it cold.*
...
- **Two-phase: audit → remediation, never mixed.** The audit produces findings, not fixes. Exception: *trivial confirmed bugs* (typo'd predicate, `x || x`) are fixed inline when the fix is smaller than the writeup, shipped as a small per-chunk **trivia code PR** into the workshop (never batched into the findings branch's commits — the findings branch stays code-free; revised 2026-06-10).
- **Rolling anchor.** Each chunk audits the **workshop's tip** (`audit/integration` — see Branch/PR flow) as of when that chunk is claimed; the runner merges `dev` into the workshop at claim time, so a chunk reads Joe's latest work *plus* every fix shipped so far. Every finding cites `file:line` @ that SHA. Joe's dev work continues mid-audit — except the two big structural refactors (`step()` phase-handler refactor, `engine.js` decomposition), which stay parked until the findings inform their seams.
...
2. **Structural footguns** — patterns that breed future bugs: one fact maintained in N hand-synced places (the `makeCard` whitelist class), silent-failure paths, copy-paste divergence. Includes **comment hygiene** (~21% of proto lines are comments, measured 2026-06-10): a comment that *contradicts* the code is a finding (it misleads every future reader, including the ship gate's intent reads) — **and it gets fixed in remediation through the normal ladder**, on whichever side is actually lying: if the code matches canon, the stale comment is corrected (docs-only, no behavior change — textbook ship-class); if the comment captured the intent and the code drifted, it's a code bug finding with the comment as the canon hint; if canon can't adjudicate which side was meant, it stages as a decision packet for Joe. Purely redundant narration gets flagged for one-time pruning in remediation. Comments are NOT stripped for the audit itself — they're the stated-intent layer the audit compares behavior against, and stripping breaks `file:line` citations.
...
**Dedupe rule:** anything already tracked in `reference/html-proto/BACKLOG.md` or `docs/DIVERGENCE.md` is cited, not re-filed — likewise anything already fixed in the workshop or covered by an earlier finding (belt-and-braces: chunks read the workshop, so re-finds should be rare) — but the audit may *upgrade* an existing item ("this deferred wart is live, here's the repro").
...
- **`/audit-next-chunk` runner** (project skill, built in Phase 0): read `docs/audit/STATE.md` → claim next chunk (merge `dev` into the workshop; record the workshop anchor SHA) → execute at the chunk's tier → write findings file → **self-QA gate** → commit to the audit branch → update STATE.md → loop to next chunk. Runs greedily until the queue or the usage budget is exhausted.
...
- **Branch/PR flow — the workshop model (revised 2026-06-10):** two long-lived Claude-owned branches. **`audit/findings`** (docs only): findings accumulate commit-by-commit; one PR to dev grows as chunks land, reviewable as a document. **`audit/integration`** (the **workshop**, code): branched from dev at campaign start; every fix is built as its own small PR targeting the workshop, evidence attached, and **the robot merges it there itself** — fix PRs are a browsable paper trail, not morning chores. The runner merges `dev` into the workshop at least at every chunk claim, so it never drifts far from Joe's live work. **Joe's code gate moves to the workshop's exit:** on request or at campaign milestones the robot prepares one `audit/integration` → `dev` PR, and Joe gives it the **ultimate review** before merge (his words, 2026-06-10) — that door stays CODEOWNERS-gated, unchanged. Tradeoff accepted: fixes batch up in the workshop rather than trickling into dev daily, so day-to-day play-testing keeps its bugs until an integration PR lands; the P0 escape hatch bypasses this. Morning review never gates overnight progress.
- **Branch ownership (Joe, 2026-06-10):** the `audit/*` branch namespace (`audit/findings`, the `audit/integration` workshop, plus per-fix `audit/fix-*` branches) is **Claude-owned** — Claude is authorized to create, push, pull, **open and self-merge PRs within them**, and otherwise manage these branches autonomously; that is the whole reason they exist. (Verified 2026-06-10: the protection ruleset covers only `main` + `dev`, so robot-merging inside `audit/*` needs no settings change.) The boundary is unchanged: nothing reaches `dev` except via PR through Joe's CODEOWNERS gate.
...
**Nightly ordering:** ship-class fixes run first — small, and because they robot-merge into the workshop immediately, later chunks always read already-fixed code — then the run resumes the chunk queue. Fix PRs (including the per-chunk trivia PRs) are separate from the `audit/findings` docs branch: findings PRs stay reviewable as documents, fix PRs as code.
...
2. Create `docs/audit/` + `STATE.md` skeleton (queue table: chunk, status todo/in_progress/done, claim timestamp, anchor SHA, findings link); create the `audit/integration` workshop branch off dev; set up **two dedicated worktrees** (one pinned to `audit/findings` for docs, one for workshop fix work — no branch switching, per the clobbering footgun).
3. **Stand up mutation testing** (Stryker or a hand-rolled mutant runner against `tests/run_all.js`) and kick off the first full run — the coverage map must exist before chunk 1, because the ship gate reads it. The full-codebase upfront run is deliberate (Joe, 2026-06-10): it doubles as the **malformed-test filter**, identifying fake-green tests suite-wide before any chunk leans on them — *identify and discount* up front; actually fixing a weak test is chunk/remediation work, since a strengthened test can go red against a real engine bug. Schedule nightly mutation + selfplay sweeps (pure machine time).
4. Write the `/audit-next-chunk` project skill encoding the runner loop above, written against the pre-mortem table below.
5. Pre-authorize the audit worktrees' permission allowlist — **enumerate every predictable permission upfront**; the dry run serves as the completeness check. This includes **wiring the promptless push/PR transport** per IDENTITIES.md → Push/PR flow (Update 2026-06-10): the per-worktree **keyring-derived-token credential helper** (exact config block in IDENTITIES.md; note gh's own helper only serves the *active* account — falsified live 2026-06-10), plus a `gh-bot` wrapper script for API writes. No bespoke token-in-URL commands — mechanism beats instruction.
6. Dry-run on chunk 6 (stickers), supervised. **Arming is automatic ("Just Say Go," 2026-06-10):** if the dry run's hard mechanical assertions pass — zero permission prompts, full commit→push→PR cycle, **identity assert** (PR author == bot account), self-QA green on the findings artifact — the runner arms the queue itself and says so; any failure leaves it unarmed with a report. No human-flipped switch.
...
| New plumbing, first contact | Supervised dry run + the loud calibration window (first nights' workshop merges flagged for close reading) | Expect 1–3 plumbing bugs the first night or two — visible and cheap by design, never silent |
