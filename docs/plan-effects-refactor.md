...
**Effort estimate:** **L** (~5.5–6 days end-to-end across both engines, ~80–85 hours, including card migration, the §3.9 mana deep-clean + full-convergence Godot land-as-ability adoption, the human `chooses()` prompt, tests, splice helper extraction, and registry consolidation; this is by far the largest of the planned refactors because it touches all 258 proto cards, all 32 Godot templates, and rewrites the dispatch table itself).
