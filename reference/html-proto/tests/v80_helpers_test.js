...
check('appendMergedText removed (§3.10 — staple text regenerated by describeCardText)',
  !/function appendMergedText\b/.test(code));

console.log('\n=== Source-level: call site counts ===');
const lppb = (code.match(/leavesPlayPreservingBuffs\(/g) || []).length;
check('leavesPlayPreservingBuffs calls (>=4 call sites + def + comments)',
  lppb >= 4, 'actual=' + lppb);

const amt = (code.match(/appendMergedText\(/g) || []).length;
check('no appendMergedText calls remain (§3.10)', amt === 0, 'actual=' + amt);
...
check('Inline merged.text concat pattern gone (§3.10 removed appendMergedText)',
  oldMergedAppend === 0, 'found ' + oldMergedAppend);
