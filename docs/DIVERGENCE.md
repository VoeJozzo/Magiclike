...
| # | Area | Godot | Proto | Tag | TO-DO |
|---|---|---|---|---|---|
| D0 | Stack resolution order + post-cast priority | LIFO via `Stack.push` / `Stack.pop_top` (engine.gd:628, 669). Caster retains priority after cast (engine.gd:685-686 in `_do_cast_spell`), per MTG 117.1c / canon §603. | LIFO via `G.stack.push` / `G.stack.pop` (engine.js:5007, 5030). **Diverges on priority:** after a stack push the proto hands priority to the NON-caster (`pushOnStack` engine.js:5012, `pushTriggerEntry` engine.js:3396 — deliberate per the design comment at 4913-4917), so the opponent gets the first response window; the caster regains priority only after the opponent passes. | ✅ LIFO same / 🔴 priority diverges | **Open adjudication — audit finding A1-1 (staged):** either the proto adopts caster-retains (canon §603, matches Godot) or the house rule is kept and canon + Godot change. Update this row when A1-1 lands. |
