...
| righteous_cavalry | round 19; W2C2 vanilla Human Knight 4/3. User: "2.x is better" → DECISIVE control pool win | keeper 2.04 / gen_04 (seed 1957601849) = control → righteous_cavalry/art.png. Quality flags: 1.04 (treatment) + 2.03 (control) | **control** (decisive pool; keeper 2.04) |

**Keeper art placed (html-proto `art.png`):** worldly_tutor (2.02/control), righteous_cavalry (2.04/control), vine_twister (1.02/treatment), vengeful_spirit (2.04/treatment), squire_of_oaths (2.06/control) — plus reassigned frames garruks_companion + sage_of_the_wilds. drain_life: no keeper (no art placed).
