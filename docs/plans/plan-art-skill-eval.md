...
| worldly_tutor | round 18; G1 tutor sorcery (search creature → hand). User: "2.x is overall better quality" → DECISIVE control pool win | keeper 2.02 / gen_02 (seed 1821500172) = control → worldly_tutor/art.png. Per user, two treatment frames reassigned to other cards: 1.02 / gen_02 (seed 1821500172) → garruks_companion/art.png; 1.04 / gen_04 (seed 1140651905) → sage_of_the_wilds/art.png | **control** (decisive pool; keeper 2.02) |
| righteous_cavalry | round 19; W2C2 vanilla Human Knight 4/3. User (revised): "no clear winner" — can't decide 2.04 vs 1.04 → TIE (no pool difference) | art retained = 2.04 / gen_04 (seed 1957601849) = control (earlier "best" pick); 1.04 = treatment | **tie** (no pool diff); art 2.04 |
| awaken_the_vault | round 20; G1C2 animate-land sorcery (a land becomes a 3/3). User: "1.01 is the best… 1.x is better" → DECISIVE treatment pool win | keeper 1.01 / gen_01 (seed 213936396) = treatment → awaken_the_vault/art.png | **treatment** (decisive pool; keeper 1.01) |
| wind_dancer | round 21; U1 vanilla Faerie 1/1 flying. User: "neither arm better or even terribly good" → TIE (no pool difference). API-gen accounting: arm_a 6=6, arm_b 4=4, 0 discarded | soft pick 1.06 / gen_06 (seed 1808370322) = control → wind_dancer/art.png (user noted not great) | **tie** (no pool diff); art 1.06 |
| crusaders_charm | round 22; W2C1 modal sorcery flash (Smite/Embolden/Sanctuary). User: "2 is better" → DECISIVE control pool win. API-gen accounting: arm_a 6=6, arm_b 6=6, 0 discarded | keeper 2.06 / gen_06 (seed 293175437) = control → crusaders_charm/art.png | **control** (decisive pool; keeper 2.06) |
| ember_anvil | round 23; Artifact Land—Mountain (taps R). User: "1.x is better" → DECISIVE treatment pool win. API-gen accounting: arm_a 5=5, arm_b 7=7, 0 discarded | keeper 1.02 / gen_02 (seed 2049778635) = treatment → ember_anvil/art.png | **treatment** (decisive pool; keeper 1.02) |
| ember_herald | round 24; R1C1 Human Shaman 1/2, ETB deal 1. User: "no pref between branches" → TIE (no pool difference). API-gen accounting: arm_a 6=6, arm_b 6=6, 0 discarded | pick 2.04 / gen_04 (seed 1340080029) = control → ember_herald/art.png | **tie** (no pool diff); art 2.04 |
| tide_charm | round 25; U1C2 modal sorcery flash (Counter/Bounce/Foresee). User: "no clear better branch" → TIE (no pool difference). API-gen accounting: arm_a 6=6, arm_b 7=7, 0 discarded | keeper 2.01 / gen_01 (seed 1915507057) = treatment → tide_charm/art.png | **tie** (no pool diff); art 2.01 |
| sengir_vampire | round 26; B2C3 Vampire 4/4 flying lifelink, grows on kill. User: "2.x is better" → DECISIVE control pool win. API-gen accounting: arm_a 8=8, arm_b 8=8, 0 discarded | keeper 2.01 / gen_01 (seed 1645105290) = control → sengir_vampire/art.png | **control** (decisive pool; keeper 2.01) |
| faithless_looting | round 27; R1 sorcery (draw 2, discard 2). User: "2.x is the best" → DECISIVE control pool win. API-gen accounting: arm_a 4=4, arm_b 4=4, 0 discarded | keeper 2.03 / gen_03 (seed 939632014) = control → faithless_looting/art.png | **control** (decisive pool; keeper 2.03) |
| embargo | round 28 (elystra replacement); W1C1 special sorcery (bounce + cost sticker). User: "hard to pick a branch" → TIE (no pool difference). API-gen accounting: arm_a 5=5, arm_b 5=5, 0 discarded | keeper 2.04 / gen_04 (seed 436155797) = treatment → embargo/art.png | **tie** (no pool diff); art 2.04 |
| primal_roar | round 29; G1C2 sorcery (pump all yours +2/+2). User: "1.x generally better" → DECISIVE treatment pool win. API-gen accounting: arm_a 6=6, arm_b 5=5, 0 discarded | keeper 1.02 / gen_02 (seed 586564502) = treatment → primal_roar/art.png | **treatment** (decisive pool; keeper 1.02) |
| sicken | round 30; B1 sorcery (flash), -2/-2 to target opp creature. User: "1.x is much, much better" → DECISIVE control pool win. API-gen accounting: arm_a 7=7, arm_b 5=5, 0 discarded | keeper 1.07 / gen_07 (seed 189633194) = control → sicken/art.png | **control** (decisive pool; keeper 1.07) |
| counter_specialist | round 31; U1C2 Human Wizard 1/4, grows +1/+1 permanent per counterspell cast. User: "1.x is the better branch" (1.06 also high quality) → DECISIVE control pool win. API-gen accounting: arm_a 8=8, arm_b 5=5, 0 discarded | keeper 1.08 / gen_08 (seed 1974560675) = control → counter_specialist/art.png | **control** (decisive pool; keeper 1.08) |
| predate | round 32; G1C1 sorcery (pump your creature +1/+1, then fight an opp creature). User: "2.06, easy. 2.x better" → DECISIVE treatment pool win. API-gen accounting: arm_a 8=8, arm_b 6=6, 0 discarded | keeper 2.06 / gen_06 (seed 756725803) = treatment → predate/art.png | **treatment** (decisive pool; keeper 2.06) |
| soulblade_captain | round 33; W1C2 Human Knight 2/2, +1/+0 to all yours on each of your spell casts. User: "1.03 is the keeper, can't pick a better overall pool" → TIE (no pool difference). API-gen accounting: arm_a 6=6, arm_b 5=5, 0 discarded | keeper 1.03 / gen_03 (seed 1618044514) = control → soulblade_captain/art.png | **tie** (no pool diff); art 1.03 |
| brand_of_iron | round 34; R1 sorcery (target creature permanently becomes an Artifact). User: "1.03 is the keeper. no overall better pool" → TIE (no pool difference). API-gen accounting: arm_a 6=6, arm_b 6=6, 0 discarded | keeper 1.03 / gen_03 (seed 1241165565) = treatment → brand_of_iron/art.png | **tie** (no pool diff); art 1.03 |
| flame_lash | round 35; R1C2 sorcery (4 damage to target creature). User: "2.04. 2.x is better overall" → DECISIVE control pool win. API-gen accounting: arm_a 8=8, arm_b 6=6, 0 discarded | keeper 2.04 / gen_04 (seed 1051637264) = control → flame_lash/art.png | **control** (decisive pool; keeper 2.04) |
| storm_sage | round 36; U2C2 Human Wizard 2/3, draw a card when another creature you control enters. User: "1.02, 1.x is better" → DECISIVE treatment pool win. API-gen accounting: arm_a 6=6, arm_b 6=6, 0 discarded | keeper 1.02 / gen_02 (seed 539635545) = treatment → storm_sage/art.png | **treatment** (decisive pool; keeper 1.02) |

**Keeper art placed (html-proto `art.png`):** worldly_tutor (2.02/control), righteous_cavalry (2.04/control), vine_twister (1.02/treatment), vengeful_spirit (2.04/treatment), squire_of_oaths (2.06/control), awaken_the_vault (1.01/treatment), wind_dancer (1.06/control, soft), crusaders_charm (2.06/control), ember_anvil (1.02/treatment), ember_herald (2.04/control), tide_charm (2.01/treatment), sengir_vampire (2.01/control), faithless_looting (2.03/control), embargo (2.04/treatment), primal_roar (1.02/treatment), sicken (1.07/control), counter_specialist (1.08/control), predate (2.06/treatment), soulblade_captain (1.03/control), brand_of_iron (1.03/treatment), flame_lash (2.04/control), storm_sage (1.02/treatment) — plus reassigned frames garruks_companion + sage_of_the_wilds. drain_life: no keeper (no art placed).

_Note: elystra_the_immortal was drawn (salt c4-28) but SKIPPED — it is a special `art_ladder` card already fully arted (art-1/2/3.png); the harness `unarted_pool()` only checked for a single `art.png` and missed it. Harness fixed to exclude art_ladder / art-*.png cards. Removed from done_c4; replacement drawn._
