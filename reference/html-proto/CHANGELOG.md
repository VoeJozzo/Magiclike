...
v2.2.47: Pacifism's authored face now states its existing opponent-only target
restriction: “Target creature an opponent controls can't attack or block.” Its
`opp_creature` targeting and two keyword-grant effects are unchanged. A focused
golden follows the shared card view-model path and pins the displayed wording.
Suite 151 files / 3005 green; lint clean.
