...
- **Rename `.v2-*` CSS classes** — `.card-v2`, `.v2-pip`, `.v2-pt`, `.v2-name`, `.v2-cost`, `.v2-art`, `.v2-type`, `.v2-text`, `.v2-oracle`, `.v2-stickers`, `.v2-title`, `.v2-bumped`, `.v2-damage`, `.v2-cardback` are now historical baggage (no v1 frame to disambiguate against). Drop the `v2-` prefix entirely (`.card-frame`, `.card-pip`, etc.) in a single dedicated commit. Large blast radius across `magiclike_engine.html` + `render.js` + `controller.js`, zero behavior change. Pure rename.
