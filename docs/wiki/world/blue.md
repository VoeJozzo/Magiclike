---
type: concept
tags: [magiclike, worldbuilding, color-pie, draft]
created: 2026-06-09
updated: 2026-06-09
---

# Blue — knowledge burns

## Philosophy *(authorial)*

> "One can serve knowledge, one can revel in the wonder of the world, but knowledge *burns*. And when ideals are betrayed… one can sometimes snap." — Joe

Blue's two faces are both already in the corpus: the **wonder** (a young wizard with glasses falling off his face, libraries, dice cast before a throne, an astral self stepping out with anticipation) and the **burn** (what knowing costs, what losing the archive does to the people who served it).

## The library, and after *(observed)*

The corpus contains a complete blue story, told in small windows:

- **Before:** Studious Research, Archivist, Lore Seeker — one ancient library, cluttered tabletops, scroll racks. Plausibly the same sloppy-haired young wizard at two ages, and his elder. Status: same-institution reading is adopted-by-vibe; same-*person* reading is unconfirmed and fine to leave that way.
- **The fire:** Martyred Scholar — a scholar dying on marble stairs while invading soldiers in red robes and iron armor sack the library. The prompt names its scene: the destruction of the Library of Alexandria.
- **After:** Prodigal Sorcerer — "a young wizard exiled from his order," staff barely sparking, walking a gray road. Quickdraw Mage — teleporting into feasts like a fugitive. Archmage of Veils — face hidden. Every post-fire blue card is a different answer to *what do you do when the shelves burn?*

This is the set's clearest faction-scale story and it should stay scattered across small windows rather than ever getting a summary card.

## Texture notes

- Blue is not all dim libraries — it has royal halls, courts, feasts (Divination's throne room, Preordain's cathedral font). Don't let the underlit mood ([[art-language]] §1) flatten blue into one room.
- Blue's relationship to [[the-divine]] is *procedural*: Divination and Preordain both show clergy/magi performing rites that visibly function as *information* — whatever the gods are, the asking works even when the answering is ambiguous.

## For card-makers

Blue cards want: the wonder paid for; the specific texture of institutions of knowing (catalogues, oaths, marginalia, provenance); exiles carrying one salvaged thing ([[threads]] — Aether Voyager clutches a single glowing scroll). Blue's snap — the betrayed idealist — is its bridge to both [[white]]'s erosion and [[black]]'s ledger, and probably where its scariest cards live.

Parked name: "the Pharos Order" for the burned library's order (lighthouse of Alexandria; an institution of *kept light*) — unreviewed; see [[names-ledger]].

## See also

[[world-bible|world hub]] · [[red]] (the arsonists) · [[threads]] · [[tone]]
