...
v2.2.47: the campaign map now names its exit boss persistently in the map
header, from first node through the boss encounter. The label and existing
node tooltips share one constructed-deck identity resolver; sectors with no
single resolved boss omit the header rather than presenting a misleading
identity. Map progression coverage pins one named exit boss per sector and
