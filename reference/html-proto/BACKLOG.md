...
- **Route `stickers.js` type mutations through the `types.js` accessor layer** -- `stickers.js` still reaches straight into the raw `types[]` array to append rolled subtypes, bypassing the accessor layer (`hasType` / `typesOf`) that `types.js` declares as the type-identity source of truth. Fix: add a small add-tag helper to `types.js` and route the sticker subtype writes and staple-merge subtype unions through it. This is currently owned by PR #60; leave this item here until that PR lands.
