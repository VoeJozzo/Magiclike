...
| [A2-2](chunk-02-combat.md) | P1 | combat | engine.js | lethalNeeded==0 blocker (fully-marked indestructible, or A1-3 survivor) classed "unsatisfied" — all trample carryover suppressed + whole remainder dumped on it (found by all 4 lenses, 4 live repros) | stage | open |
...
| [A2-4](chunk-02-combat.md) | P2 | combat | engine.js | declareAttackers legality accepts duplicate iids (blockers side has the Set) — one creature attacks N times, N× damage + N× 'attacks' triggers; engine API surface only | stage | open |
