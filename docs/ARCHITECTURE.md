...
| `js/triggers.js` | 133 | `TRIGGER_CONDITIONS` registry (25+ condIds). `evalTriggerCondition` resolver. |
...
**Effect dispatch.** ~40 handler kinds keyed off `effect.kind`. Handler signature `(ctx, params, target) => void`. Notable handlers:
- `damage` — `applyDamageFrom` tracks `damagedBySources`, checks indestructible
- `removeCreature` — severity-tiered (1=tap, 2=bounce, 3=destroy, 4=exile); bounce preserves `permaBuffs`
- `endomorphAbsorb` — reads victim keywords, applies sticker to controller's slot via `RUN.applyStickerToSlot()`
- `embargo`/`bleach`/`steal` — mutate slot-level fields persisted by RUN
...
| Triggered abilities | ✓ Phase 4 | ✓ | Godot has 1 predicate, proto has 25+. |
