...
// decision 2). damage/pump/removeCreature gain a `scope` path
...
console.log('\n=== removeCreature(severity:1 tap, scope: all_opps) ===');
(() => {
  clearBoards();
  const a = place('you'), b = place('opp');
  ENGINE.applyEffect(CTX, { kind: 'removeCreature', severity: 1, scope: 'all_opps' }, null);
  check('opp creature tapped', b.tapped === true);
  check('your creature NOT tapped', a.tapped === false);
})();

console.log('\n=== removeCreature(severity:3 destroy, scope: all_creatures) ===');
(() => {
  clearBoards();
  place('you'); place('opp'); place('opp');
  ENGINE.applyEffect(CTX, { kind: 'removeCreature', severity: 3, scope: 'all_creatures' }, null);
...
(() => {
  clearBoards();
  const a = place('you');
  ENGINE.applyEffect(CTX, { kind: 'removeCreature', severity: 1 }, { kind: 'creature', iid: a.iid });
