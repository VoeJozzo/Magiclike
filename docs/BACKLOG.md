...
### Tooling / process
- **Stale-comment flag from the comment sweep: `scenes/game/game_board.gd:1001`.** The comment claims the castable-highlight defers to `can_pay`, but the lines below compute `surplus_in_pool` explicitly. Decide which is intended (comment truth-fix vs simplifying the code to defer); left in place as evidence.
- **Godot loose-ends scan (post-proto-audit, maybe).** During the html-proto audit planning (2026-06-09) the Godot port was deliberately excluded from audit scope — it isn't worth rebuilding yet. Parked here: a *light* pass over the Godot side for obvious dead threads to snip (vestigial wiring, stale TODOs, half-removed seams), explicitly NOT a comprehensive audit. Consider after the proto audit's remediation phase, when the port's re-sync against the cleaned proto is being planned.
